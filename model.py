...
import csv
"""pre-process"""
ds = csv.reader(open(os.path.join(__file__, '..', 'data',
                                  'CTUStrike_analysis_file_public_release.csv'), 'rb'))
...
#                tweet.append(ngram)
        ...
        try:
            ...
            corpus['full'].append(tweet)

            if float(d_row['position_mean']) > 4:
                corpus['against'].append(tweet)
            elif float(d_row['position_mean']) < 2:
                corpus['for'].append(tweet)
            elif float(2.75 <= d_row['position_mean']) <= 3.25:
                corpus['neutral'].append(tweet)
...
for train_index, test_index in strat_split:
    for _train_i in train_index:
        X_train.append(unicode(corpus['full'][_train_i], "ISO-8859-1"))
        y_train.append(labels[_train_i]) 
    for _test_i in test_index:
        X_test.append(unicode(corpus['full'][_test_i], "ISO-8859-1"))
        y_test.append(labels[_test_i])
        

for key in corpus.iterkeys():
    with closing(open(os.path.join(__file__, '..', 'data', 'corpus_%s.pkl' % key), 'wb')) as pkl:
        cPickle.dump(corpus[key], pkl)
